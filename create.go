...
// CreateWithTemplateInModule writes a new blank migration file in specific module
func CreateWithTemplateInModule(db *sql.DB, dir string, tmpl *template.Template, name, migrationType, module string) error {
	var version string
	if sequential {
		// always use DirFS here because it's modifying operation
		migrations, err := collectMigrationsFS(osFS{}, dir, minVersion, maxVersion)
	...
	if tmpl == nil {
		if migrationType == "go" {
			tmpl = goSQLMigrationModuleTemplate
		} else {
			tmpl = sqlMigrationTemplate
		}
	}
...
var goSQLMigrationModuleTemplate = template.Must(template.New("goose.go-migration").Parse(`package migrations
...
func init() {
	goose.AddModuleMigration("{{.Module}}", up{{.CamelName}}, down{{.CamelName}})
}
